/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
/*
 * HandlerNvpRegisterServer.cpp - _explain_
 *
 * Copyright (C) 2011 ezlibs.com, All Rights Reserved.
 *
 * $Id: HandlerNvpRegisterServer.cpp 5884 2013-01-29 03:21:10Z WuJunjie $
 *
 *  Explain:
 *     -explain-
 *
...
CHandlerNvpRegisterServer::CHandlerNvpRegisterServer(CMaintainAgentLog *pLog)
        :SocketHandler(pLog)
{
...
CStreamContainer *CHandlerNvpRegisterServer::GetPandoraContainer()
{
...
#define SIZE 5000
void CHandlerNvpRegisterServer::tprintf(TcpSocket *p,const char *format, ...)
{
    va_list ap;
    size_t n;
    char tmp[SIZE];
...
void CHandlerNvpRegisterServer::List(TcpSocket *sendto)
{
    int ii=0;
    tprintf(sendto, "HandlerNvpRegisterServer Socket List\n");
    tprintf(sendto, "---------------------------------------------------\n");
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++, ii++)
    {
        Socket *p = (*it).second;
        TcpSocket *p3 = dynamic_cast<TcpSocket *>(p);
        //CHttpServerNvpRegisterServer *p4 = dynamic_cast<CHttpServerNvpRegisterServer *>(p);
...
void CHandlerNvpRegisterServer::List(std::string &strDump, int iLevel)
{
    int ii=0;
    char sendto[1024];

    snprintf(sendto, sizeof (sendto)-1, "HandlerNvpRegisterServer Socket List\n");
    strDump = sendto;
    snprintf(sendto, sizeof (sendto)-1, "---------------------------------------------------\n");
    strDump += sendto;
    int iNumOfCHttpServerNvpRegisterServer = 0;
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++, ii++)
    {
        Socket *p = (*it).second;
        TcpSocket *p3 = dynamic_cast<TcpSocket *>(p);
        ...
        if (p4)
        {
            iNumOfCHttpServerNvpRegisterServer++;
        }
...
//            }
    ...
    if (iLevel==enLogLevelShow_contes)
    {
        snprintf(sendto, sizeof (sendto)-1, "        -------------------------------------------\n");
        strDump += sendto;
        snprintf(sendto, sizeof (sendto)-1, "\tCHttpServerNvpRegisterServer: [%d]\n", iNumOfCHttpServerNvpRegisterServer);
        strDump += sendto;
    }
...
void CHandlerNvpRegisterServer::Update()
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        Socket *p = (*it).second;
        CHttpServerNvpRegisterServer *pNvpRegisterServer = dynamic_cast<CHttpServerNvpRegisterServer *>(p);

        if (pNvpRegisterServer)
        {
            pNvpRegisterServer->Update();
        }
    }
}

#if defined(_FUNC_NvpRegisterServer_SingleServer)
CStreamContainer *CHandlerNvpRegisterServer::GetPandoraContainer4Channel()
{
    return m_pChannelContainer;
}
