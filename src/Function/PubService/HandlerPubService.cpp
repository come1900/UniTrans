...
CHandlerPubService::CHandlerPubService(CMaintainAgentLog *pLog)
        :SocketHandler(pLog)
{
    ;
}
...
void CHandlerPubService::List(std::string &strDump, int iLevel)
{
    int ii=0;
    char sendto[1024];

    snprintf(sendto, sizeof (sendto)-1, "HandlerPubService Socket List\n");
    strDump = sendto;
