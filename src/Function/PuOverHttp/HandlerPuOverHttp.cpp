/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
/*
 * HandlerPuOverHttp.cpp - _explain_
 *
 * Copyright (C) 2011 ezlibs.com, All Rights Reserved.
 *
 * $Id: HandlerPuOverHttp.cpp 5884 2013-01-29 03:21:10Z WuJunjie $
 *
 *  Explain:
 *     -explain-
 *
...
CHandlerPuOverHttp::CHandlerPuOverHttp(CMaintainAgentLog *pLog)
        :SocketHandler(pLog)
{
    ;
}
...
#define SIZE 5000
void CHandlerPuOverHttp::tprintf(TcpSocket *p,const char *format, ...)
{
    va_list ap;
    size_t n;
    char tmp[SIZE];
...
void CHandlerPuOverHttp::List(TcpSocket *sendto)
{
    int ii=0;
    tprintf(sendto, "HandlerPuOverHttp Socket List\n");
    tprintf(sendto, "---------------------------------------------------\n");
...
void CHandlerPuOverHttp::List(std::string &strDump, int iLevel)
{
    int ii=0;
    char sendto[1024];

    snprintf(sendto, sizeof (sendto)-1, "HandlerPuOverHttp Socket List\n");
    strDump = sendto;
...
void CHandlerPuOverHttp::Update()
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        Socket *p = (*it).second;
        CPuOverHttpKeepConnector *p3 = dynamic_cast<CPuOverHttpKeepConnector *>(p);
...
Socket * CHandlerPuOverHttp::GetSocketByName(std::string strServerName)
{
    //int ii=0;
    ...
    DEB(printf("CHandlerPuOverHttp::GetSocketByName(%s) failed.\n", strServerName.c_str()););

    return NULL;
}

CPuOverHttpKeepConnector * CHandlerPuOverHttp::GetSocketBySessionID(std::string strSessionID)
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        Socket *p = (*it).second;
    ...
    DEB(printf("CHandlerPuOverHttp::GetSocketBySessionID(%s) failed.\n", strSessionID.c_str()););

    return NULL;
}

// ¼øÈ¨
int CHandlerPuOverHttp::AuthSession(std::string strSession, int iAccessId)
{
