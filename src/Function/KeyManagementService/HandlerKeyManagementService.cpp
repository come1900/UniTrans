/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
/*
 * HandlerKeyManagementService.cpp - _explain_
 *
 * Copyright (C) 2011 jsict.com, All Rights Reserved.
 *
 * $Id: HandlerKeyManagementService.cpp 5884 2013-01-29 03:21:10Z WuJunjie $
 *
 *  Explain:
 *     -explain-
 *
...
#define def_keyManagementServiceKeyValue "keyValue_keyManagementService.s3"

#define ARG_USED(x) (void)&x;

CHandlerKeyManagementService::CHandlerKeyManagementService(CMaintainAgentLog *pLog)
...
CHandlerKeyManagementService::~CHandlerKeyManagementService()
{
    if (m_pCacheWithDb)
    {
...
#define SIZE 5000
void CHandlerKeyManagementService::tprintf(TcpSocket *p,const char *format, ...)
{
    va_list ap;
    size_t n;
...
void CHandlerKeyManagementService::List(TcpSocket *sendto)
{
    int ii=0;
    tprintf(sendto, "HandlerKeyManagementService Socket List\n");
    tprintf(sendto, "---------------------------------------------------\n");
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++, ii++)
    {
        Socket *p = (*it).second;
        TcpSocket *p3 = dynamic_cast<TcpSocket *>(p);
        //CHttpServerKeyManagementService *p4 = dynamic_cast<CHttpServerKeyManagementService *>(p);
...
void CHandlerKeyManagementService::List(std::string &strDump, int iLevel)
{
    int ii=0;
    char sendto[1024];

    snprintf(sendto, sizeof (sendto)-1, "HandlerKeyManagementService Socket List\n");
    strDump = sendto;
    snprintf(sendto, sizeof (sendto)-1, "---------------------------------------------------\n");
    strDump += sendto;
    int iNumOfCHttpServerKeyManagementService = 0;
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++, ii++)
    {
        Socket *p = (*it).second;
        TcpSocket *p3 = dynamic_cast<TcpSocket *>(p);
        ...
        if (p4)
        {
            iNumOfCHttpServerKeyManagementService++;
        }
...
//            }
    ...
    if (iLevel==enLogLevelShow_contes)
    {
        snprintf(sendto, sizeof (sendto)-1, "        -------------------------------------------\n");
        strDump += sendto;
        snprintf(sendto, sizeof (sendto)-1, "\tCHttpServerKeyManagementService: [%d]\n", iNumOfCHttpServerKeyManagementService);
        strDump += sendto;
    }
...
void CHandlerKeyManagementService::Update()
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        Socket *p = (*it).second;
        CHttpServerKeyManagementService *pKeyManagementService = dynamic_cast<CHttpServerKeyManagementService *>(p);

        if (pKeyManagementService)
        {
            pKeyManagementService->Update();
        }
    }
}

void CHandlerKeyManagementService::Start()
{
    if (NULL == m_pCacheWithDb)
    {
        ...
        //strDbFileName+="/";
        //strDbFileName+=def_keyManagementServiceKeyValue;

        CConfigGeneral __cfgGeneral;
        __cfgGeneral.update();

        m_pCacheWithDb->setDbfilename((__cfgGeneral.getConfig().strDataFilePath + std::string("/") + def_keyManagementServiceKeyValue));
        //m_pCacheWithDb->setDbfilename(def_keyManagementServiceKeyValue);
